(async function(){
  ...
  function card(p){return `
    <div class="card">
      ...
      <script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@type":"Product","name":p.name,"brand":p.brand,"offers":{"@type":"Offer","priceCurrency":"EUR","price":p.price}})}</script>
    </div>`;}

  function render(){
    const list=data.filter(p=>!activeTags.size || (p.tags||[]).some(t=>activeTags.has(t)));
  ...
  go.onclick=()=>{
    const chosen=data.filter(p=>selected.has(p.id));
